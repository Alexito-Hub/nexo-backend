...
#     E.g.: docker.io/hexpm/elixir:1.19.5-erlang-28.5-debian-trixie-20260610-slim
...
# `ca-certificates` es obligatorio: el driver de Mongo verifica el certificado
# de Atlas contra el almacén de CA del sistema. `curl` sirve al HEALTHCHECK.
RUN apt-get update \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/nexo ./

USER nobody

EXPOSE 4000

# La sonda considera sano al servicio solo si además responde MongoDB.
